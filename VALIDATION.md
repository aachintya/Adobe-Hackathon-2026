...
The September 11 task-sampling update adds resumable task-directed collection, conservative table relationships, exact-quote locator hints, readiness consistency checks and an independently adjudicated report scorer. Both fresh final-revision Luna audits completed within five-minute observer bounds (246 seconds on a corrected healthy fixture; 277 seconds on BBG). The healthy report preserved fully supported answers and zero findings; BBG still exposed an unsupported answer element. Earlier interrupted runs, an invalid control and all report-level judgments are preserved under `validation/TASK-SAMPLING-2026-09-11.md` and `validation/task-sampling-2026-09-11/` in the repository, excluded from the ZIP. The full deterministic suite passed, including 4 task-sampling, 8 relationship, 11 evaluator, 33 evidence-gate and 15 finalizer tests plus extracted-package execution. These results do not establish actual Adobe judge score improvement.
