...
def render(case, path, query):
    ...
    if path == 'index.html':
        ...
        if kind == 1:
            ...
            body += '<p>Autumn evening language courses for adult learners. Before applying, check the enrollment deadline and eligibility in our enrollment guide.</p>'
        ...
        body = '<h1>Application steps</h1><p>Read the enrollment guide, choose a class, then visit our public reception desk with proof of age. No online submission is required. Reception opens weekdays 09:00–17:00.</p>' + link('sable-archive.html', 'Enrollment guide')
    elif kind == 3 and path in {'pricing.html','products.html','team.html','security.html','news.html'}:
        body = '<h1>' + path.split('.')[0].title() + '</h1><p>General school information. For course enrollment dates and eligibility, use the enrollment guide.</p>' + link('sable-archive.html', 'Enrollment guide')
