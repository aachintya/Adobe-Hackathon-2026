...
def main():
    ...
    comparable=all(before.get(key)==after.get(key) for key in ("collector","collector_version","site","limits"))
    result={"site":after.get("site"),"comparable":comparable,"warnings":[],"changes":[]}
    if not comparable: result["warnings"].append("Collector version, mode, limits or site differs; do not attribute changes to the site.")
    ...
    for url in sorted(set(pb)&set(pa)):
        x,y=pb[url],pa[url]
        for field in ("status","final_url","canonical","meta_robot_directives","x_robots_tags"):
