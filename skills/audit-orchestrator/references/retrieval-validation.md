...
Before the shared evidence deadline, use at most 30 seconds for 3-4 queries: exact entity name; one neutral category/task query without the brand; and 1-2 material factual lookups derived from the question tests. Batch independent queries when supported and shorten this stage if less time remains; record any unexecuted checks. Record exact queries, date, tool, locale (or unknown), returned source URLs and short evidence quotes from opened sources as you go. Compare same-entity claims with the site's current statements. Keep this compact ledger in the shared working evidence and summarize findings/readiness with supporting URLs and query details. For automated report verification, retain opened external passages as host observations in the review sidecar; do not write a duplicate narrative ledger. The report remains the default deliverable.
