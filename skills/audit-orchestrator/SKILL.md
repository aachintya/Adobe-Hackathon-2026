...
8. Emit ONE JSON report conforming to [report-schema.json](references/report-schema.json). Replace the static draft with the composed findings, all five readiness stages, 3-5 question tests and two journeys; do not change the contract to save time. Set `assessment.mode` to `agent_composed`; keep all unavailable lanes explicit. `assessment.visibility` stays `not_measured` unless actual recorded assistant runs support it. Recompute counts and sequential IDs after ranking. Prefer three well-supported questions over five shallow ones. Use a two-sentence executive conclusion, one concise observation per record, short material quotes, and one reproduction per root cause. Preserve all verified material findings and actionable fixes; remove repeated narrative, not supporting facts. Keep query/source notes in the shared working evidence; write no separate narrative ledger unless requested. Aim to have the report ready by elapsed 240 seconds, leaving 40 seconds for validation and delivery. No extra prose unless requested.
