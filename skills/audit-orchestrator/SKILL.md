...
9. If Python exists, validate the saved report with `python <marketplace-root>/tests/validate_report.py REPORT.json`; correct errors within the reserved time. Do not run the development test suite during a website audit. Complete the remaining reasoning stages when the host agent has the necessary capabilities.
