...
For each question, fill its `assessment.intent_tests` record using `main_text`, `passages`, actual JSON-LD values, media alternatives and relevant linked pages. Cite a short quote and URL for each answer; record partial or untested coverage honestly. A long marketing page may lack the important answer; a short clear page may answer it fully. Compare rendered and source facts when a browser can resolve uncertainty. Review `evidence_truncated` before claiming absence.
