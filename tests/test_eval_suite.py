...
def main():
    ...
    assert suite.get("skill_name")=="audit-orchestrator" and len(cases)>=8
    assert [case.get("id") for case in cases]==list(range(1,len(cases)+1))
    assert all(case.get("prompt") and case.get("expected_output") and len(case.get("assertions",[]))>=3 for case in cases)
    text=json.dumps(cases).lower()
    required={"rendered","entity ambiguity","path-preserving","sitemap","multilingual","images and pdfs","healthy","five-minute"}
